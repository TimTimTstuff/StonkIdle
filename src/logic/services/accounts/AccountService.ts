...
export class AccountService implements IGameService{
    ...
    constructor(saveService:SaveDataService, event:GlobalEvents, time:TimeService) {
    ...
    private recalculateTax(log:Taxlog) {
        log.totalIncome = (log.sellShare + log.interest)-log.buyShare
        log.cost = (log.totalIncome * GameConfig.tax)*-1
    }
    ...
    public addMainAccount(amount:number, reason?:string):boolean {
        this._saveService.getGameSave().player.mainAccount.balance += amount
